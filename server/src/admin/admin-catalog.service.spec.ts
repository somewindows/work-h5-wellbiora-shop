...
describe('AdminCatalogService', () => {
  ...
  it('草稿允许保存缺脚注的半成品，发布时才拒绝没有来源脚注的星号数据宣称', async () => {
    ...
    await expect(service.saveDraftBlocks('p1', blocks)).resolves.toMatchObject({ id: 'p1' })
    await expect(service.publishDraft('p1', actor)).rejects.toMatchObject({ code: 42201 })
  })

  it('草稿允许保存缺少必填字段的半成品，发布时才校验必填', async () => {
    const { service } = await createService()

    await expect(service.saveDraftBlocks('p1', [{ type: 'image' }])).resolves.toMatchObject({ id: 'p1' })
    await expect(service.publishDraft('p1', actor)).rejects.toMatchObject({ code: 42201 })
  })
  ...
  it('保存草稿时即拒绝非法块类型与缺少 type 的内容块', async () => {
    ...
    await expect(service.saveDraftBlocks('p1', [{ type: 'unknown_block' }])).rejects.toMatchObject({ code: 42201 })
  ...
  it('局部更新保留未提交的字段（DTO 实例的 undefined 属性不覆盖原值）', async () => {
    ...
    const seeded = await repository.findById('p1')
    ...
    const saved = await service.updateProduct('p1', dto, actor)

    expect(saved).toMatchObject({ id: 'p1', name: '测试商品', priceFen: 200, theme: '#000000', flavor: '莓果', goodsNo: 'G-1', isActive: true })
  })
  ...
  it('新建商品拒绝重复 ID', async () => {
    const { service } = await createService()
    const dto = plainToInstance(CreateAdminProductDto, {
      id: 'p1', name: '重复', en: 'Dup', priceFen: 100,
  ...
  it('发布时保留前后版本并记录操作日志', async () => {
    ...
    await service.publishDraft('p1', actor)
  ...
  it('回滚到上一发布版并记录新版本与审计日志', async () => {
    ...
    await service.saveDraftBlocks('p1', [{ type: 'gallery', images: ['/assets/next.jpg'] }])
    await service.publishDraft('p1', actor)

    const rolledBack = await service.rollback('p1', actor)
    ...
    await expect(versions.findByProduct('p1')).resolves.toMatchObject([{ version: 1 }, { version: 2 }, { version: 3 }])
    await expect(auditLogs.findByTarget('catalog_product', 'p1')).resolves.toMatchObject([
      { action: 'publish' },
  ...
  it('没有上一发布版时回滚报错', async () => {
    ...
    await expect(service.rollback('p1', actor)).rejects.toMatchObject({ code: 40002 })
  })
})
