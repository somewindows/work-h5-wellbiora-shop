...
describe('购物车接口（e2e）', () => {
  ...
  it('登录后可加入、修改和移除购物车商品', async () => {
    ...
    const add = await agent.post('/api/v1/cart/items').set('Authorization', `Bearer ${token}`).send({ productId: 'WB10001', quantity: 1 }).expect(201)
    ...
    expect(list.body.data).toMatchObject([{ id: itemId, productId: 'WB10001', quantity: 2, checked: false }])
