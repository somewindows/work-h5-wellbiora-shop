...
- **T13 · 项目复审修复**（2026-09-12 立项）：依据 `docs/project-review/2026.09.12-project-review.md`（R01~R17）+ `2026.09.12-deepseek-verification.md`（核实 + 严重性重排 + 4.1~4.5 新增发现）。两份文档结论可信，但**行号/路径有约十处偏移，修每条前按描述重新定位真实代码**。修支付/退款/回调必须过「支付链路评审关」（派子代理对抗性评审）+ 先读 `docs/pitfalls/2026-09-12-微信支付联调避坑.md`
  - **跨机器交接速览（2026-09-13 深夜更新）**：第 0/1/2 批全部完成，第 3 批完成 4.3（剩 R06/R10/R15，R07 依赖君梦），第 4 批杂项未动。换电脑后 `git pull` 即可继续。**服务器部署待办（按序）**：① `git pull` → ② `cd server && npm run build && npm run migration:run`（refunds 新表，迁移铁律必须真实跑）→ ③ 重启 NSSM 服务 → ④ `cd admin && npm run build` → ⑤ `robocopy dist D:\www\wellbiora\site\admin /MIR /NFL /NDL /NJH /NJS`（admin 前端含退款记录 UI）→ ⑥ 可选：admin 后台对真实订单做 0.01 退款，验证退款状态机+回调/收敛链路
  - **第 0 批 · 立即（风险正在发生）**
  ...
  - **第 2 批 · 资金正确性**：~~R08 金额校验改用 `amount.total`~~（2026-09-12 已修：回调/查单均取 amount.total 校验，payer_total 不一致仅 warn；遗留：**用户实付/优惠额不落库、currency 未核验**——对账缺持久化依据，正式收款前补）；~~R03 下单事务化~~（2026-09-12 已修：主单+明细+删购物车+事件包进 runInTransaction，ER_DUP_ENTRY 精确捕获后重读幂等返回；遗留：真实 MySQL 复测回滚语义）；~~R09 取消与支付协调~~（2026-09-12 已修：取消时 best-effort 关微信交易；已取消订单的迟到扣款**登记支付事实**（paymentStatus=paid+systemRemark+事件+logger.error）不推仓不报关，转人工退款，后台因此解锁退款入口；过支付链路对抗性评审；遗留：**handleWechatPaid/cancel 仍是读旧对象整体覆盖写，并发窗口未关**——回调在 cancel 读后写前读到 pay 态会覆盖 cancelled 并推仓报关，需条件更新 `WHERE status='pay' AND payment_status='pending'` 或 @VersionColumn 乐观锁（涉及迁移，正式收款前必做；R04 退款 settleOrder 旧对象覆盖写同一根因，合并处理）；另 systemRemark 复用导致后台误显「海关拦截」标签，后续拆字段或改文案）；**R04+R05 退款单状态机 + 稳定退款单号**（2026-09-13 已修：refunds 账本表（迁移 1710000010000）+ RefundService 状态机——受理只置在途、到账以回调/查退款收敛；订单按账本重算 paymentStatus（全额=refunded/在途=refunding/部分成功=paid），支持部分退款再退；稳定退款单号 R+订单号+序号发起前落库，重复点击/重试/并发/网络异常全部收敛到一笔，通道报错先 queryRefund 按原单号查真实状态；商户平台发起的退款回调补登；admin 取消改按剩余可退发起且退款校验先于撤仓；降级护栏防旧单复活；后台详情加退款记录表+「退款中」状态+剩余可退口径；过两轮对抗性评审；**遗留：迁移铁律——真实 MySQL 跑 migration:run（本机无 MySQL，待服务器验证）**）
  - **第 3 批 · 合规与设施**：~~4.3 后台任务调度设施~~（2026-09-13 已修：`TaskSchedulerService`（进程内 setInterval + 启动 5 秒补偿轮 + 异常隔离 + unref，刻意不引入持久化队列，任务全部「扫库+幂等收敛」）+ **超时关单 job**（24h 待支付自动取消，兑现 H5 文案；取消走条件更新 `WHERE status='pay' AND payment_status='pending'` 竞态安全——回调先登记支付的订单影响 0 行自动让位，不回滚支付事实）+ **退款在途收敛 job**（回调丢失按原单号 queryRefund 收敛；超 1h 查无置 failed 释放额度允许重发；本地 mock 无 queryRefund 时空转）；env 新增 `ORDER_EXPIRE_HOURS=24`/`ORDER_EXPIRE_CHECK_INTERVAL_MS`/`REFUND_SETTLE_CHECK_INTERVAL_MS`（NaN/非正数回落默认）；单测 169/169、e2e 43/43、lint/build 过，两轮对抗性评审（阻断项=旧对象覆盖写已改条件更新）；遗留：多实例部署会重复扫描（当前单实例无影响，扩容前需加锁/选主）、取消+事件非原子（仅影响可观测性，可用已有 runInTransaction 包一层））；R06 年度额度预占；R10 推仓/报关解耦；R15 商品编号并发 + 发布竞态；R07 剩余（真实仓储/实名核验，依赖君梦接入）
