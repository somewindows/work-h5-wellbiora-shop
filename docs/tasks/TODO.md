...
- **T13 · 项目复审修复**（2026-09-12 立项）：依据 `docs/project-review/2026.09.12-project-review.md`（R01~R17）+ `2026.09.12-deepseek-verification.md`（核实 + 严重性重排 + 4.1~4.5 新增发现）。两份文档结论可信，但**行号/路径有约十处偏移，修每条前按描述重新定位真实代码**。修支付/退款/回调必须过「支付链路评审关」（派子代理对抗性评审）+ 先读 `docs/pitfalls/2026-09-12-微信支付联调避坑.md`
  - **跨机器交接速览（2026-09-15 深夜更新）**：第 0/1/2 批全部完成（含 R09 并发窗口，纯条件更新无迁移）；第 3 批完成 4.3 + R06 年度额度预占 + R15 编号并发/条件发布，**只剩 R10 推仓/报关解耦**（与 OMS 无关可直接做，下一件就是它）；R07 等 OMS 服务商确定（负责人透露可能换掉君梦，适配层设计使换仓代码零返工，OMS 侧代码暂停）；第 4 批杂项未动。换电脑 `git pull` 即可继续。~~服务器部署（含 R09/R06/R15 三个 commit）+ 真实 MySQL 冒烟三项~~（**2026-09-16 全部通过**：①同证件并发两单一成一败 GET_LOCK 锁路径正常 ②后台并发建品编号不重复/并发发布仅一个成功 ③0.01 真实单支付回归正常）。真实退款链路 2026-09-14 已验收（¥0.01 三秒到账；NOT_ENOUGH 坑已入 pitfalls）。自助清关已开通，待用户设置 APIv2 密钥填入 .env（WXPAY_API_V2_KEY/WXPAY_CUSTOMS_CODE=HANGZHOU_ZS/WXPAY_MCH_CUSTOMS_NO=5101960X8F）后报关即通
  - **第 0 批 · 立即（风险正在发生）**
