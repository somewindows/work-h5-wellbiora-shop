...
- **T12 · 微信支付模块**（2026-09-10 开发中；参数已全部就绪：AppID/AppSecret/mchid 1117333649/v3 密钥/证书）
  ...
  - [ ] 真实联调（部署后，**进行中，2026-09-11 已跑到真实支付成功**）：
    ...
    - 自助清关**已开通并打通申报提交**（2026-09-16）：APIv2 密钥已设置并填入服务器 .env（WXPAY_API_V2_KEY + WXPAY_CUSTOMS_CODE=HANGZHOU_ZS + WXPAY_MCH_CUSTOMS_NO=5101960X8F），首笔真实申报随 0.01 单自动提交（WB202609160CA38556E58E，身份校验 SAME）；**但海关回执 = 申报异常（EXCEPT），待排查**——商户平台查询页无详情（受理海关须选「杭州（总署版）」才查得到记录），已新增后台订单详情「查询报关状态」按钮（GET /admin/orders/:orderNo/customs-declaration，拉海关原始回执字段）用于取 EXCEPT 原因；怀疑方向：企业跨境电商备案未在杭州关区生效 / 备案号口径不符 / 测试单金额过小。查到原因后回填 pitfalls
