...
4. ~~后台功能完善（用户管理/管理员账号/订单导出对账/数据概览/图片上传）~~ **全部完成并部署生产**（2026-09-17~18，验收记录见 T13 交接速览；各条目核销见 `docs/requirements/2026-09-16-后台功能完善规划.md`）
5. **当前代码侧无可排期事项**——T13 清零 + 后台规划清零。剩余记录项（不阻塞，顺手做）：事件与主单/账本更新非原子（可用已有 runInTransaction 包一层）、systemRemark 复用导致后台误显「海关拦截」（拆字段或改文案）、多实例部署重复扫描任务（扩容前处理）、商品列表内存分页（规模增长再改）
6. **等外部条件（阻塞中，非代码问题）**：君梦 OMS 测试环境接入（等 appId/appSecret/shopId/warehouseNo + 商品 goods_no + OMS 服务商确定）；179 数据抓取平台对接（商务/关务，支付单申报 EXCEPT 与订单/运单申报的共同前置）；真实短信验证码通道（等签名/模板审核，届时只写一个 adapter）
7. **上线前人工/运营项**：T9 前端视觉走查（逐页目检，T9 唯一剩余项）；MySQL 每日备份定时任务注册（部署指南 17.3，脚本已就绪只需 Register-ScheduledTask）；客服/协议占位文案（等负责人提供）；4 款商品真实定价替换占位价
