...
- **T13 · 项目复审修复**（2026-09-12 立项）：依据 `docs/project-review/2026.09.12-project-review.md`（R01~R17）+ `2026.09.12-deepseek-verification.md`（核实 + 严重性重排 + 4.1~4.5 新增发现）。两份文档结论可信，但**行号/路径有约十处偏移，修每条前按描述重新定位真实代码**。修支付/退款/回调必须过「支付链路评审关」（派子代理对抗性评审）+ 先读 `docs/pitfalls/2026-09-12-微信支付联调避坑.md`
  - **跨机器交接速览（2026-09-16 晚更新）**：第 0/1/2 批全部完成（含 R09 并发窗口，纯条件更新无迁移）；第 3 批完成 4.3 + R06 年度额度预占 + R15 编号并发/条件发布，第 3 批 R10 推仓/报关解耦 2026-09-17 完成、第 4 批杂项与 R11/R12 支付链路加固 2026-09-18 全部收尾（见 archive/2026-09-18-R11R12支付加固.md），**T13 代码项已清零，剩余全是等外部条件项**；R07 等 OMS 服务商确定（负责人透露可能换掉君梦，适配层设计使换仓代码零返工，OMS 侧代码暂停）。换电脑 `git pull` 即可继续。**R09/R06/R15 已于 2026-09-16 部署服务器并真实 MySQL 冒烟三项全过**（同证件并发一单一败 / 并发建品编号不重复、并发发布仅一个成功 / 0.01 支付回归）。**自助清关链路已全通**：APIv2 密钥已配，首笔申报 WB202609160CA38556E58E 身份校验 SAME；后台「查询报关状态」工具已上线并经真实环境修复（customs 漏传 + 序号后缀解析，`ce7a689`→`fe250a2`），**EXCEPT 根因已查明 = 179 数据抓取平台未对接，属商务/关务动作，代码无解**（详见 T12 末节与 `docs/pitfalls/2026-09-16-微信报关查询避坑.md`）。支付链路评审关记录：本次改动已过对抗性评审（PASS 无阻断）；遗留建议（不阻塞）：① AdminOrderDetail 加 hasWechatTransaction 让前端按钮显隐与服务端口径一致（本地 mock 支付单点查询会 40002，文案已兜住）② 新路由无 e2e 覆盖 ③ 微信请求无超时（全局既有模式）。**2026-09-16 晚更新**：报关查询功能上线并修复 customs 漏传（`fe250a2`），EXCEPT 根因查明 = 179 数据抓取平台未对接（商务推进中，见 T12 条目）；后台查看类长内容统一右侧抽屉 + UI 规范入 admin/README（`bbabb57`）；**后台功能完善规划已立项 → `docs/requirements/2026-09-16-后台功能完善规划.md`**（P0=R10，P1=用户管理/管理员账号/ICP 页脚，启动哪条先转 TODO）
  - **第 0 批 · 立即（风险正在发生）**
  ...
  - **第 2 批 · 资金正确性**：~~R08 金额校验改用 `amount.total`~~（2026-09-12 已修：回调/查单均取 amount.total 校验，payer_total 不一致仅 warn；~~遗留：用户实付/优惠额不落库、currency 未核验~~ **2026-09-17 已关闭**：payer_total_fen/pay_currency 落库 + 订单导出 CSV，见 archive/2026-09-17-订单导出与对账落库.md）；~~R03 下单事务化~~（2026-09-12 已修：主单+明细+删购物车+事件包进 runInTransaction，ER_DUP_ENTRY 精确捕获后重读幂等返回；遗留：真实 MySQL 复测回滚语义）；~~R09 取消与支付协调~~（2026-09-12 已修：取消时 best-effort 关微信交易；已取消订单的迟到扣款**登记支付事实**（paymentStatus=paid+systemRemark+事件+logger.error）不推仓不报关，转人工退款，后台因此解锁退款入口；过支付链路对抗性评审；**并发窗口 2026-09-15 已关**：handleWechatPaid/cancel/confirmMockPayment/refund settleOrder 全部改条件更新（`WHERE status='pay' AND payment_status='pending'` 等，复用 cancelIfPendingPayment 模式，无迁移）——落败方重读分流：取消晚于支付抛 40002、回调晚于取消撤孤儿推仓+登记支付事实不复活订单、重复回调幂等；admin cancel 待支付落败自动转 paid 分支撤仓+退款；settleOrder 重读+定向更新三列不再整体覆盖；新增 8 个竞态用例，单测 177/177、e2e 43/43，对抗性评审 PASS；遗留记录项：① 并发重复回调会在写库前重复 pushOrder（mock 幂等无害，**真实 OMS 推单幂等性待确认，并入 R10**）② 事件与主单非原子（原实现既有，可观测性缺口，可用 runInTransaction 包一层）③ admin cancel paid 分支仍是整体写小窗口（毫秒级，可覆盖并发 sync 的 warehouseStatus/systemRemark）；另 systemRemark 复用导致后台误显「海关拦截」标签，后续拆字段或改文案）；**R04+R05 退款单状态机 + 稳定退款单号**（2026-09-13 已修：refunds 账本表（迁移 1710000010000）+ RefundService 状态机——受理只置在途、到账以回调/查退款收敛；订单按账本重算 paymentStatus（全额=refunded/在途=refunding/部分成功=paid），支持部分退款再退；稳定退款单号 R+订单号+序号发起前落库，重复点击/重试/并发/网络异常全部收敛到一笔，通道报错先 queryRefund 按原单号查真实状态；商户平台发起的退款回调补登；admin 取消改按剩余可退发起且退款校验先于撤仓；降级护栏防旧单复活；后台详情加退款记录表+「退款中」状态+剩余可退口径；过两轮对抗性评审；**遗留：取消+退款事件与账本更新非原子（仅影响可观测性）**；~~迁移铁律~~已兑现：服务器真实 MySQL 跑过 migration:run，2026-09-14 真实退款 ¥0.01 三秒到账全链路验收通过）
  - **第 3 批 · 合规与设施**：~~4.3 后台任务调度设施~~（2026-09-13 已修：`TaskSchedulerService`（进程内 setInterval + 启动 5 秒补偿轮 + 异常隔离 + unref，刻意不引入持久化队列，任务全部「扫库+幂等收敛」）+ **超时关单 job**（24h 待支付自动取消，兑现 H5 文案；取消走条件更新 `WHERE status='pay' AND payment_status='pending'` 竞态安全——回调先登记支付的订单影响 0 行自动让位，不回滚支付事实）+ **退款在途收敛 job**（回调丢失按原单号 queryRefund 收敛；超 1h 查无置 failed 释放额度允许重发；本地 mock 无 queryRefund 时空转）；env 新增 `ORDER_EXPIRE_HOURS=24`/`ORDER_EXPIRE_CHECK_INTERVAL_MS`/`REFUND_SETTLE_CHECK_INTERVAL_MS`（NaN/非正数回落默认）；单测 169/169、e2e 43/43、lint/build 过，两轮对抗性评审（阻断项=旧对象覆盖写已改条件更新）；遗留：多实例部署会重复扫描（当前单实例无影响，扩容前需加锁/选主）、取消+事件非原子（仅影响可观测性，可用已有 runInTransaction 包一层））；~~R06 年度额度预占~~（2026-09-15 已修：下单即预占（负责人确认口径）——`sumOccupiedYearlyFen` 占用集合改为 (pending+pay) 或 paid/refunding，取消（含 24h 超时关单）与全额退款随查询语义自动释放、部分退款不释放；年度归属=创建年（注释写明，海关口径上线前与服务商确认）；并发原子化用 `withYearlyQuotaLock`（MySQL GET_LOCK 按实名指纹命名锁，不同账号同证件共享额度；锁内重查 requestId 幂等；锁超时转业务码 40001；InMemory 用 Promise 链互斥镜像语义）；预检仍查做 UX 提示、支付回调不复查（创建已占住）；新增 7 用例，单测 184/184、e2e 43/43，对抗性评审 PASS；**GET_LOCK 锁路径真实 MySQL 冒烟 2026-09-16 已通过**（同证件并发两单一成一败）；记录项：锁连接≠业务写连接（临界区占 2 条池连接）、Between 边界包含性概率级发散、单库前提（读写分离/多实例锁失效））；~~R10 推仓/报关解耦~~（**2026-09-17 完成**：支付回调先条件更新登记支付事实（markPaidIfPending 不再写 warehouseStatus），推仓/报关解耦为「回调内 best-effort 一次 + OrderFulfillmentJob 扫库收敛（5 分钟/24h 窗口，env FULFILLMENT_CHECK_INTERVAL_MS/FULFILLMENT_RETRY_WINDOW_HOURS）+ 后台人工重推端点 retry-fulfillment（终态 EXCEPT/FAIL 先条件重置再重申报）」；orders 新列 customs_declare_status/customs_declared_at（迁移 1710000011000，便携 MySQL run/revert/run 已验）；cancel 已支付分支撤仓决策改用退款后重读快照、未推仓跳过撤单；TaskScheduler 加单任务重叠保护；后台订单详情加「海关申报」状态卡+「重推推仓/报关」按钮。单测 217/217、e2e 43/43、lint/build、admin type-check/build 全绿；两轮对抗性评审（首轮 FAIL 四个阻断项：并发双推仓误撤/取消单被自动报关/终态人工重推不可达/撤仓用陈旧快照——全部修复并补回归用例，复审 PASS）。**部署：必须跑 migration:run（有新列）**。遗留记录项：① 退款通道 failed 回 paid 与 job 毫秒窗的残余夹缝（概率极低，可改条件更新根治）② convergeDeclaration 事件与落库在窄竞态下可能背离（仅事件流误导）③ R09-① 真实 OMS 推单幂等约束已写入 warehouse.adapter.ts 注释，接真实 OMS 时列为验收项）；~~R15 商品编号并发 + 发布竞态~~（2026-09-15 已修：编号「分配+insert-only 插入」撞 ER_DUP_ENTRY 重分配重试闭环（上限 5 次耗尽 40002，最小空号复用规则不变，杜绝 save() 主键覆盖）；发布改**条件发布**——服务层把已校验快照+期望 contentVersion 传仓储，事务内 SELECT FOR UPDATE 锁行重读校验版本+内容摘要（canonicalJson 键序无关）一致才写，快照/版本同事务，回滚同模式；新增 5 用例，单测 189/189、e2e 43/43，对抗性评审 PASS；语义收紧：重复/过期发布收 40002 提示刷新而非静默成功；**insert()/FOR UPDATE/事务多写三条新 SQL 路径真实 MySQL 冒烟 2026-09-16 已通过**（并发建品/并发发布）；记录项：审计在主写事务外可能丢审计）；R07 剩余（真实仓储/实名核验，依赖 OMS 服务商确定）
  - **第 4 批 · 收尾**：**2026-09-18 已收尾**（明细见 archive/2026-09-18-T13第四批收尾杂项.md）——R17 备份脚本改 `--result-file` + 便携 MySQL 恢复演练通过（16/16 表）；qs 6.15.3→6.16.0 + multer 2.2.0→2.4.0 overrides（server audit 清零）；Swiper 未使用已移除（frontend critical 告警消除）；H5 401 自动登出（request.ts 全局拦截跳登录带回跳）；结算页金额改用服务端预检快照展示；登录页 mock 提示按开关区分；订单详情区分 404 与网络错误；admin JWT 独立化（ADMIN_JWT_SECRET 可配 + 12h 有效期，撤销靠守卫回查库）；全局异常过滤器补 5xx 定位日志。**遗留**：客服/协议占位（等负责人提供内容）；商品列表内存分页（4 款商品无影响，规模增长再改）；vite/vitest/esbuild 开发链 audit 告警（仅开发期、修复需大版本升级，接受）
  - **R11/R12 支付链路加固**：**2026-09-18 已完成**（明细见 archive/2026-09-18-R11R12支付加固.md）——V3 成功应答强制验签后才采信（公钥/证书双模式，拉证书应答按微信指引豁免）、请求超时兜底（WXPAY_REQUEST_TIMEOUT_MS 默认 15s，覆盖 body 读取）、查单/查退款应答关联字段核对（out_trade_no/mchid/appid/out_refund_no）、平台证书刷新 single-flight + 60s 最小间隔 + 失败退避（含全部解密失败按失败处理）；对抗性评审 PASS 后按建议修复 4 项；单测 279/279、e2e 60/60、lint/build 全绿
...
- **T12 · 微信支付模块**（2026-09-10 开发中；参数已全部就绪：AppID/AppSecret/mchid 1117333649/v3 密钥/证书）
  ...
  - [ ] 真实联调（部署后，**进行中，2026-09-11 已跑到真实支付成功**）：
    ...
    - 自助清关**已开通并打通申报提交**（2026-09-16）：APIv2 密钥已设置并填入服务器 .env（WXPAY_API_V2_KEY + WXPAY_CUSTOMS_CODE=HANGZHOU_ZS + WXPAY_MCH_CUSTOMS_NO=5101960X8F），首笔真实申报随 0.01 单自动提交（WB202609160CA38556E58E，身份校验 SAME）；后台「查询报关状态」已取到海关原始回执（修复 customs 必填漏传后生效，见 `docs/pitfalls/2026-09-16-微信报关查询避坑.md`）。**EXCEPT 原因已查明**：海关回执 explanation = 「电商平台[5101960X8F]未按照海关总署179号公告要求完成跨境进口数据抓取平台的对接」——微信→海关通道已通（备案号/签名/身份校验 SAME 全过），是**平台企业未完成杭州海关数据抓取平台对接**（179 号公告：海关须能实时抓取平台订单原始数据）。**待办（商务/关务，非代码）**：179 对接责任主体 = 泽芃铭自己（平台编码持有方，保税仓/君梦无处置权只能代办）；实操委托君梦/保税仓/报关行代办，需提供营业执照 + 电子口岸法人卡/操作员卡（IC 卡）+ 海关备案信息，负责人已在联系保税仓（2026-09-16）；此为订单/运单申报（君梦 OMS）共同前置条件
...
3. ~~R10 推仓/报关解耦~~ **已完成**（2026-09-17，见 T13 第 3 批）；~~R11/R12 支付链路加固~~ **已完成**（2026-09-18，见 archive/2026-09-18-R11R12支付加固.md）——T13 代码项全部清零
4. 后台功能完善：按 `docs/requirements/2026-09-16-后台功能完善规划.md` 优先级启动（P1 = 用户管理 / 管理员账号 / ICP 页脚；P2 = 订单导出对账 / 数据概览 / 图片上传；启动时把条目从规划文档转入活跃任务）
5. 君梦 OMS 测试环境接入（等 appId/appSecret/shopId/warehouseNo + 商品 goods_no + 179 对接 + OMS 服务商确定）
6. 上线加固：发布事务化、admin JWT 独立 secret、MySQL 自动备份、生产密钥配置
...
- [x] 商户平台海关备案 + 自助清关开通（2026-09 已完成：杭州总署版 / 5101960X8F，APIv2 密钥已配，申报提交已通）；**剩余阻塞 = 179 数据抓取平台对接**（负责人 + 保税仓推进中，2026-09-16；责任主体 = 泽芃铭，可委托代办，详见 `docs/pitfalls/2026-09-16-微信报关查询避坑.md`）
