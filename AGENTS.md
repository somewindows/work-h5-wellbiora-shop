...
├── admin/                         # 管理后台桌面端（T10 已建）：数据概览/登录/商品管理/内容块编辑器/订单管理（含导出CSV）/用户管理/管理员账号/操作日志
