...
- **后端本地联调闭环已建立（2026-08-30，T8）**：`server/`（NestJS + TypeScript + TypeORM + MySQL 8），已实现统一 `/api/v1` 响应层、认证（开发短信验证码/JWT）、首页商品、购物车、地址实名和订单预检/创建/查询/取消；验证码与限频存 MySQL，不使用 Redis。身份证号 AES-256-GCM 密文保存、HMAC 指纹用于年度额度查询；支付和仓库均为仅本地的 mock 适配器，真实微信/君梦待接入。服务端单元 25/25、e2e 8/8、lint 和生产构建已验证通过。`admin/` 尚未创建。
