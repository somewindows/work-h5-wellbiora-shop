...
- **管理员账号**（仅超管）：列表（角色/状态/首登待改密标记）、新增（一次性临时密码仅显示一次）、禁用/启用、重置密码（新临时密码 + 强制改密）；顶栏「修改密码」改自己密码；临时密码首登被路由守卫强制引导改密。

## UI 规范（弹窗 / 抽屉，2026-09-16 定）

- **只读查看类长内容 → 右侧抽屉 `el-drawer`**：海关申报回执、操作日志变更数据、草稿 JSON 预览这类几十行键值表/JSON，统一右侧抽屉，默认 `size="640px"`，超大内容可用百分比（如 `50%`）。
- **操作/表单/确认类 → 居中对话框 `el-dialog`**：确认类（取消订单、退款）`480px`；表单类（新建商品）`640px`。
- **禁止把长只读内容塞进窄对话框**（label 会被挤压成一字一行，2026-09-16 海关回执 560px 弹窗实测踩坑）。
- 抽屉/对话框内一律文本或 `<pre>` 渲染，禁用 `v-html`（同安全约定）。
